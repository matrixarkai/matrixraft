...
use matrixraft::benchmark::{
    rustraft_assert_production_reference_raft_artifacts, RustRaftBenchmarkFailureSummary,
...
fn main() {
    ...
    match rustraft_assert_production_reference_raft_artifacts(&report, &summary) {
        Ok(()) => {
            println!("ReferenceRaft-vs-RustRaft benchmark artifacts verified");
        }
        Err(error) => {
            eprintln!("ReferenceRaft-vs-RustRaft benchmark artifacts failed: {error}");
            std::process::exit(1);
        }
    }
}
...
fn usage() {
    eprintln!(
        "Usage: reference_raft_parity_verify --report PATH --summary PATH [--max-age-seconds N]"
    );
}
...
fn assert_fresh_artifact(path: &PathBuf, label: &str, max_age_seconds: u64) {
    ...
    if age.as_secs() > max_age_seconds {
        eprintln!(
            "ReferenceRaft-vs-RustRaft benchmark artifacts failed: benchmark:artifact_stale:{label}:age_seconds={}:max_age_seconds={}",
            age.as_secs(),
            max_age_seconds
        );
        std::process::exit(1);
