...
fn persistent_wal_reports_slow_fsync_backpressure_through_lifecycle_status() {
    ...
    assert_eq!(wal.status().compacted_after_slow_fsync_count, 1);
    assert_eq!(wal.status().slow_fsync_segment_count, 1);
    assert_eq!(wal.status().compacted_slow_fsync_segment_count, 1);

    let _ = fs::remove_dir_all(dir);
}

#[test]
fn persistent_wal_compaction_after_slow_fsync_requires_releasing_a_slow_segment() {
    ...
    let released = wal.compact_through(4).expect("compact fast segments");
    assert!(released > 0);
