...
use matrixraft::benchmark::{
    ...
    rustraft_assert_production_reference_raft_summary,
    rustraft_find_or_build_reference_raft_harness, rustraft_find_reference_raft_harness,
    rustraft_probe_reference_raft_native_benchmark, rustraft_reference_raft_benchmark_evidence,
    rustraft_reference_raft_benchmark_failure_summary,
    rustraft_run_reference_raft_parity_benchmark,
    rustraft_validate_production_reference_raft_benchmark_options, RustRaftBenchmarkEngine,
    RustRaftBenchmarkEngineSource, RustRaftBenchmarkHarnessKind, RustRaftBenchmarkOptions,
    RustRaftBenchmarkRunner, RustRaftBenchmarkWorkload, RustRaftExternalReferenceRaftRunner,
...
fn temp_dir(name: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "rustraft-real-reference_raft-{name}-{}-{nonce}",
        std::process::id()
    ))
}
...
fn benchmark_artifact_verifier_rejects_debug_benchmark_environment() {
    ...
    make_fake_reference_raft_harness(&root);
    ...
    let mut reference_raft =
        RustRaftExternalReferenceRaftRunner::from_root(&root, "release").expect("runner");
    ...
    let verify_script = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("scripts")
        .join("verify_reference_raft_benchmark_artifacts.sh");
...
fn benchmark_artifact_verifier_example_rejects_invalid_env_max_age_before_parsing() {
    ...
    let output = Command::new("cargo")
        ...
        .arg("--quiet")
        .arg("--example")
        .arg("reference_raft_parity_verify")
        .arg("--")
        .arg("--report")
        .arg(&report)
        .arg("--summary")
...
#[test]
fn reference_raft_benchmark_script_verifies_successful_artifacts_before_exit() {
    let benchmark_script = fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("scripts")
            .join("reference_raft_vs_rustraft_benchmark.sh"),
    )
    ...
    assert!(benchmark_script.contains("verify_reference_raft_benchmark_artifacts.sh"));
    ...
    assert_eq!(
        benchmark_script
            .matches("echo \"ReferenceRaft benchmark harness: $reference_raft_bin\"")
            .count(),
        1
    );
}
...
fn production_benchmark_options_validator_rejects_non_release_scale_inputs() {
    ...
    let error = rustraft_validate_production_reference_raft_benchmark_options(&options)
...
#[test]
fn direct_reference_raft_benchmark_example_preflights_options_before_harness_lookup() {
    let example = fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("examples")
            .join("reference_raft_parity_benchmark.rs"),
    )
    .expect("read benchmark example");

    let preflight = example
        .find("rustraft_validate_production_reference_raft_benchmark_options(&options)")
        .expect("example should preflight production options");
    let harness_lookup = example
        .find("let reference_raft_root =")
        .expect("example should look up ReferenceRaft after option parsing");
    assert!(
        preflight < harness_lookup,
        "production option preflight must happen before ReferenceRaft harness lookup"
    );
    assert!(example.contains("ReferenceRaft parity benchmark invalid production options"));
...
#[cfg(unix)]
#[test]
fn direct_reference_raft_benchmark_example_writes_summary_atomically() {
    let root = temp_dir("direct-summary-atomic");
    let fake_bin = make_fake_reference_raft_harness(&root);
    make_fake_git_checkout(&root);
    ...
    let output = Command::new("cargo")
        ...
        .env("REFERENCE_RAFT_ROOT", &root)
        .env("REFERENCE_RAFT_BENCHMARK_BIN", &fake_bin)
        .env(
    ...
    assert_eq!(
        summary_json["schema"],
        "rustraft.reference_raft_benchmark_summary.v1"
    );
...
fn readme_documents_release_scale_benchmark_contract() {
    ...
    assert!(readme.contains("scripts/reference_raft_vs_rustraft_benchmark.sh"));
    ...
    assert!(
        readme.contains("--summary-out target/reference_raft-vs-rustraft-benchmark/summary.json")
    );
    ...
    assert!(readme.contains("scripts/verify_reference_raft_benchmark_artifacts.sh"));
}
...
#[cfg(unix)]
#[test]
fn reference_raft_benchmark_script_rejects_non_executable_harness_before_cargo_run() {
    let root = temp_dir("script-non-executable-harness");
    let reference_raft_root = root.join("reference_raft");
    let bin_dir = reference_raft_root.join("bin");
    fs::create_dir_all(&bin_dir).expect("bin dir");
    let harness = bin_dir.join("reference_raft_parity_benchmark");
    ...
    let output = Command::new("bash")
        ...
        .arg("--reference_raft-root")
        .arg(&reference_raft_root)
        .arg("--reference_raft-bin")
        .arg(&harness)
        .arg("--out")
        .arg(&report)
        .output()
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("benchmark:real_reference_raft_harness_not_executable"));
...
#[cfg(unix)]
#[test]
fn reference_raft_benchmark_script_rejects_invalid_dimensions_before_harness_lookup() {
    ...
    let output = Command::new("bash")
        ...
        .arg(env!("CARGO_MANIFEST_DIR"))
        .arg("--reference_raft-root")
        .arg(root.join("missing-reference_raft"))
        .arg("--iterations")
        .arg("0")
        .arg("--out")
        .arg(&report)
    ...
    assert!(!stderr.contains("benchmark:real_reference_raft_missing"));
    assert!(!report.exists());
    let _ = fs::remove_dir_all(root);
}
...
#[cfg(unix)]
#[test]
fn reference_raft_benchmark_script_rejects_tiny_iterations_before_harness_lookup() {
...
#[cfg(unix)]
#[test]
fn reference_raft_benchmark_script_rejects_below_scale_node_count_before_harness_lookup() {
...
#[cfg(unix)]
#[test]
fn reference_raft_benchmark_script_rejects_unbatched_batch_size_before_harness_lookup() {
...
#[cfg(unix)]
#[test]
fn reference_raft_benchmark_script_rejects_tiny_payload_before_harness_lookup() {
...
#[cfg(unix)]
#[test]
fn reference_raft_benchmark_script_rejects_loose_tolerance_before_harness_lookup() {
...
#[cfg(unix)]
#[test]
fn reference_raft_benchmark_script_rejects_invalid_tolerance_before_harness_lookup() {
...
#[cfg(unix)]
#[test]
fn reference_raft_benchmark_script_rejects_invalid_artifact_age_before_harness_lookup() {
...
#[cfg(unix)]
#[test]
fn reference_raft_benchmark_script_rejects_artifact_path_collision_before_harness_lookup() {
...
#[cfg(unix)]
#[test]
fn reference_raft_benchmark_script_rejects_empty_report_path_before_harness_lookup() {
...
#[cfg(unix)]
#[test]
fn reference_raft_benchmark_script_rejects_directory_report_path_before_harness_lookup() {
...
#[cfg(unix)]
#[test]
fn reference_raft_benchmark_script_rejects_directory_summary_path_before_harness_lookup() {
...
#[cfg(unix)]
#[test]
fn reference_raft_benchmark_script_does_not_treat_non_executable_native_bins_as_runnable() {
    let benchmark_script = fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("scripts")
            .join("reference_raft_vs_rustraft_benchmark.sh"),
    ...
    assert!(
        benchmark_script.contains("ReferenceRaft native capability: kvbench=${kvbench:-missing}")
    );
    assert!(benchmark_script.contains("export REFERENCE_RAFT_KVSERVER_BIN=\"$kvserver_candidate\""));
    assert!(benchmark_script.contains("export REFERENCE_RAFT_KVBENCH_BIN=\"$kvbench_candidate\""));
    assert_eq!(
        benchmark_script.matches("[[ -x \"$candidate\" ]]").count(),
        4,
        "ReferenceRaft benchmark, native kvserver, native kvbench, and native capability discovery should all require executable candidates"
    );
    assert!(!benchmark_script.contains("[[ -x \"$candidate\" || -f \"$candidate\" ]]"));
}

#[cfg(unix)]
fn make_fake_reference_raft_harness(root: &std::path::Path) -> PathBuf {
    let bin_dir = root.join("bin");
    fs::create_dir_all(&bin_dir).expect("bin dir");
    let bin = bin_dir.join("reference_raft_parity_benchmark");
    write_executable(
        &bin,
        r#"#!/usr/bin/env bash
set -euo pipefail
...
cat <<JSON
{
  ...
  "implementation": "reference_raft_cpp",
  "binary_path": null,
  "git_revision": "$(git -C "$(dirname "$0")/.." rev-parse HEAD)",
  "build_profile": "release",
  "harness_kind": "full_reference_raft_harness",
...
fn make_fake_git_checkout(root: &std::path::Path) {
    for args in [
        ...
        vec!["add", "."],
        vec!["commit", "-m", "fake reference_raft harness"],
    ] {
...
#[cfg(unix)]
#[test]
fn real_reference_raft_runner_uses_external_harness_and_production_sources() {
    let root = temp_dir("root");
    let fake_bin = make_fake_reference_raft_harness(&root);
    make_fake_git_checkout(&root);
    assert_eq!(
        rustraft_find_reference_raft_harness(&root).expect("find"),
        fake_bin
    );
    ...
    let mut report =
        rustraft_run_reference_raft_parity_benchmark(&mut reference_raft, &mut rustraft, &options);
    let debug_error = rustraft_assert_production_reference_raft_parity(&report)
    ...
    rustraft_assert_production_reference_raft_parity(&report).expect("production parity");
    assert!(report.generated_at_unix_ms > 0);

    let summary = rustraft_reference_raft_benchmark_failure_summary(&report);
    assert_eq!(summary.generated_at_unix_ms, report.generated_at_unix_ms);
    rustraft_assert_production_reference_raft_summary(&summary).expect("production summary");

    let evidence = rustraft_reference_raft_benchmark_evidence(&report);
    assert!(evidence.real_reference_raft);
    assert!(evidence.rustraft_runtime);
    assert!(evidence.reference_raft_cpp_reference);
    ...
    assert!(report.comparisons.iter().all(|comparison| {
        comparison.reference_raft.engine_source == RustRaftBenchmarkEngineSource::RealReferenceRaft
...
fn production_benchmark_rejects_stale_report_and_summary_timestamps() {
    ...
    let error =
        rustraft_assert_production_reference_raft_parity(&report).expect_err("stale report");
    assert!(error.contains("benchmark:report_generated_at_stale"));

    let summary = rustraft_reference_raft_benchmark_failure_summary(&report);
    let summary_error =
        rustraft_assert_production_reference_raft_summary(&summary).expect_err("stale summary");
...
fn benchmark_reports_use_unique_process_scoped_run_ids() {
    ...
    for report in [&first, &second] {
        assert!(report
            .benchmark_run_id
            .starts_with("rustraft-reference-raft-parity-"));
        assert!(
            report
                .benchmark_run_id
...
fn production_benchmark_rejects_non_finite_pass_tolerance() {
    ...
    let error =
        rustraft_assert_production_reference_raft_parity(&report).expect_err("invalid tolerance");
...
fn production_benchmark_rejects_loose_pass_tolerance() {
    ...
    let error =
        rustraft_assert_production_reference_raft_parity(&report).expect_err("loose tolerance");
    assert!(error.contains("benchmark:invalid_pass_tolerance_percent:10.100"));

    let summary = rustraft_reference_raft_benchmark_failure_summary(&report);
    let summary_error =
        rustraft_assert_production_reference_raft_summary(&summary).expect_err("loose tolerance");
...
fn production_benchmark_rejects_tiny_iteration_count() {
    ...
    let error =
        rustraft_assert_production_reference_raft_parity(&report).expect_err("tiny iterations");
    assert!(error.contains("benchmark:iterations_per_workload_below_production_min:2:128"));

    let summary = rustraft_reference_raft_benchmark_failure_summary(&report);
    let summary_error =
        rustraft_assert_production_reference_raft_summary(&summary).expect_err("tiny iterations");
...
fn production_benchmark_rejects_unbatched_batch_size() {
    ...
    let error = rustraft_assert_production_reference_raft_parity(&report)
        .expect_err("unbatched batch size must not satisfy production parity");
    assert!(error.contains("benchmark:batch_size_below_production_min:1:2"));

    let summary = rustraft_reference_raft_benchmark_failure_summary(&report);
    let summary_error = rustraft_assert_production_reference_raft_summary(&summary)
...
fn production_benchmark_rejects_below_scale_node_count() {
    ...
    let error = rustraft_assert_production_reference_raft_parity(&report).expect_err("below scale");
    assert!(error.contains("benchmark:node_count_below_production_scale:3:5"));

    let summary = rustraft_reference_raft_benchmark_failure_summary(&report);
    let summary_error =
        rustraft_assert_production_reference_raft_summary(&summary).expect_err("below scale");
...
fn production_benchmark_rejects_sample_engine_mismatch() {
    ...
    report.environment_fingerprint =
        "os=linux;arch=x86_64;target=x86_64-unknown-linux-gnu;debug_assertions=false".to_string();
    report.comparisons[0].rustraft.engine = RustRaftBenchmarkEngine::ReferenceRaft;
...
fn benchmark_evidence_classifies_operation_count_pair_mismatch_as_correctness() {
    ...
    report.comparisons[0]
        .blockers
        .push("operation_count_mismatch_reference_raft_128_rustraft_64".to_string());

    let evidence = rustraft_reference_raft_benchmark_evidence(&report);
    assert!(
        evidence.correctness_blockers.iter().any(|blocker| blocker
            .contains("benchmark:operation_count_mismatch_reference_raft_128_rustraft_64")),
        "{:#?}",
...
fn production_summary_rejects_inconsistent_workload_status() {
    ...
    report.environment_fingerprint =
        "os=linux;arch=x86_64;target=x86_64-unknown-linux-gnu;debug_assertions=false".to_string();
    let mut summary = rustraft_reference_raft_benchmark_failure_summary(&report);
    ...
    let regression_error = rustraft_assert_production_reference_raft_summary(&summary)
...
fn production_benchmark_rejects_throughput_duration_mismatch() {
    ...
    report.environment_fingerprint =
        "os=linux;arch=x86_64;target=x86_64-unknown-linux-gnu;debug_assertions=false".to_string();
    report.comparisons[0].reference_raft.total_duration_micros = 1;

    let error = rustraft_assert_production_reference_raft_parity(&report)
        .expect_err("throughput/duration mismatch must not satisfy production parity");
    assert!(
        error.contains("benchmark:reference_raft_sample_throughput_duration_mismatch"),
        "{error}"
    );
    ...
    assert!(
        summary_error.contains(
            "benchmark:summary_reference_raft_throughput_duration_mismatch:single_key_writes"
        ),
        "{summary_error}"
    );
...
#[cfg(unix)]
#[test]
fn real_reference_raft_runner_reports_harness_failure_as_workload_blocker() {
    ...
    write_executable(
        ...
        r#"#!/usr/bin/env bash
echo "simulated ReferenceRaft harness failure" >&2
exit 17
"#,
    );

    let mut reference_raft =
        RustRaftExternalReferenceRaftRunner::from_root(&root, "fake-failure").expect("runner");
    let sample = reference_raft.run_workload(
    ...
    assert!(sample.blockers.iter().any(|blocker| blocker
        .contains("benchmark:real_reference_raft_harness_failed:single_key_writes:exit_17")));
    let _ = fs::remove_dir_all(root);
}

#[cfg(unix)]
#[test]
fn real_reference_raft_runner_rejects_non_executable_harness_before_spawn() {
    ...
    let error =
        RustRaftExternalReferenceRaftRunner::new(&harness, Some(&root), "fake-non-executable")
            .expect_err("non-executable harness must be rejected");
    assert!(error.contains("benchmark:real_reference_raft_harness_not_executable"));
    let _ = fs::remove_dir_all(root);
}

#[cfg(unix)]
#[test]
fn reference_raft_harness_discovery_rejects_non_executable_candidate() {
    ...
    let error = rustraft_find_reference_raft_harness(&root).expect_err("non-executable harness");
    assert!(error.contains("benchmark:real_reference_raft_harness_not_executable"));
    let error = rustraft_find_or_build_reference_raft_harness(&root, "debug")
...
#[cfg(unix)]
#[test]
fn real_reference_raft_runner_reports_invalid_json_as_workload_blocker() {
...
"#,
    ...
    let mut reference_raft =
        RustRaftExternalReferenceRaftRunner::from_root(&root, "fake-invalid-json").expect("runner");
    let sample = reference_raft.run_workload(
    ...
    assert_eq!(sample.operation_count, 1);
    assert!(sample.blockers.iter().any(|blocker| blocker
        .contains("benchmark:real_reference_raft_harness_invalid_json:snapshot_streaming")));
    let _ = fs::remove_dir_all(root);
}

#[cfg(unix)]
#[test]
fn real_reference_raft_runner_stamps_and_validates_runner_owned_provenance() {
...
cat <<JSON
{
  ...
  "engine_source": "real_reference_raft",
  "implementation": "reference_raft_cpp",
  "binary_path": "/tmp/not-the-running-reference_raft-binary",
  "git_revision": "fake-reference_raft-revision",
  "build_profile": "release",
  "harness_kind": "full_reference_raft_harness",
  "node_count": 3,
  "iterations_per_workload": 1,
...
"#,
    ...
    let mut reference_raft =
        RustRaftExternalReferenceRaftRunner::from_root(&root, "debug").expect("runner");
    let sample = reference_raft.run_workload(
        RustRaftBenchmarkWorkload::SingleKeyWrites,
        &RustRaftBenchmarkOptions {
            iterations_per_workload: 1,
    ...
    assert!(sample.blockers.iter().any(|blocker| {
        blocker.contains(
            "benchmark:real_reference_raft_harness_binary_path_mismatch:single_key_writes",
        )
    }));
    assert!(sample.blockers.iter().any(|blocker| {
        blocker.contains(
            "benchmark:real_reference_raft_harness_git_revision_mismatch:single_key_writes",
        )
    }));
    assert!(sample.blockers.iter().any(|blocker| {
        blocker.contains(
            "benchmark:real_reference_raft_harness_build_profile_mismatch:single_key_writes:release:debug",
        )
    }));
...
#[cfg(unix)]
#[test]
fn real_reference_raft_runner_rejects_invalid_latency_order() {
...
"#,
    ...
    let mut reference_raft =
        RustRaftExternalReferenceRaftRunner::from_root(&root, "fake-test").expect("runner");
    let sample = reference_raft.run_workload(
        RustRaftBenchmarkWorkload::SingleKeyWrites,
        &RustRaftBenchmarkOptions {
            iterations_per_workload: 1,
    ...
    assert!(
        sample.blockers.iter().any(|blocker| {
            blocker.contains(
            "benchmark:real_reference_raft_harness_latency_order_invalid:single_key_writes:300:200",
        )
        }),
        "{sample:#?}"
    );
...
#[cfg(unix)]
#[test]
fn real_reference_raft_runner_rejects_malformed_success_sample_shape() {
...
"#,
    ...
    assert!(sample.blockers.iter().any(|blocker| blocker.contains(
        "benchmark:real_reference_raft_harness_workload_mismatch:batched_writes:single_key_writes"
    )));
    assert!(sample.blockers.iter().any(|blocker| {
        blocker.contains("benchmark:real_reference_raft_harness_engine_mismatch:batched_writes")
    }));
    assert!(sample.blockers.iter().any(|blocker| {
        blocker
            .contains("benchmark:real_reference_raft_harness_engine_source_mismatch:batched_writes")
    }));
    assert!(sample.blockers.iter().any(|blocker| {
        blocker.contains(
            "benchmark:real_reference_raft_harness_implementation_mismatch:batched_writes:model:reference_raft_cpp",
        )
    }));
    assert!(sample.blockers.iter().any(|blocker| {
        blocker.contains(
            "benchmark:real_reference_raft_harness_node_count_mismatch:batched_writes:1:5",
        )
    }));
    assert!(sample.blockers.iter().any(|blocker| {
        blocker.contains(
            "benchmark:real_reference_raft_harness_operation_count_mismatch:batched_writes:999:4",
        )
    }));
    assert!(sample
        .blockers
        .iter()
        .any(|blocker| blocker.contains("benchmark:real_reference_raft_harness_p50_latency_zero")));
    assert!(sample.blockers.iter().any(
        |blocker| blocker.contains("benchmark:real_reference_raft_harness_throughput_invalid")
    ));
...
#[cfg(unix)]
#[test]
fn reference_raft_runner_builds_harness_from_checkout_hook_before_failing_closed() {
    use std::os::unix::fs::PermissionsExt;

    let root = temp_dir("build-hook");
    let scripts = root.join("scripts");
    fs::create_dir_all(&scripts).expect("scripts");
    let build_script = scripts.join("build_reference_raft_parity_benchmark.sh");
    fs::write(
        &build_script,
        r#"#!/usr/bin/env bash
set -euo pipefail
mkdir -p bin
cat > bin/reference_raft_parity_benchmark <<'HARNESS'
#!/usr/bin/env bash
cat <<JSON
{
  "workload": "single_key_writes",
...
JSON
HARNESS
chmod +x bin/reference_raft_parity_benchmark
"#,
    )
    .expect("build script");
    ...
    let built =
        rustraft_find_or_build_reference_raft_harness(&root, "debug").expect("build fake harness");

    assert_eq!(built, root.join("bin/reference_raft_parity_benchmark"));
    assert!(built.is_file());
    let _ = fs::remove_dir_all(root);
}

#[test]
fn missing_reference_raft_harness_fails_closed_with_real_reference_raft_blocker() {
    let root = temp_dir("missing");
    fs::create_dir_all(&root).expect("root");
    let err = rustraft_find_reference_raft_harness(&root).expect_err("missing harness");
    assert!(err.contains("benchmark:real_reference_raft_missing"));
    let err = rustraft_find_or_build_reference_raft_harness(&root, "debug")
...
#[test]
fn native_reference_raft_kvbench_is_reported_as_partial_not_production_parity() {
    ...
    let capability = rustraft_probe_reference_raft_native_benchmark(&root);
    ...
    assert!(capability
        .blockers
        .contains(&"benchmark:reference_raft_kvserver_binary_missing".to_string()));
    assert!(capability
        .blockers
        .contains(&"benchmark:reference_raft_kvbench_binary_missing".to_string()));
    assert!(capability
        .blockers
        .contains(&"benchmark:reference_raft_native_kvbench_partial".to_string()));

    let err =
        rustraft_find_or_build_reference_raft_harness(&root, "debug").expect_err("partial kvbench");
    assert!(err.contains("benchmark:real_reference_raft_missing"));
...
#[test]
fn native_reference_raft_probe_reports_runnable_partial_workloads_when_kv_binaries_exist() {
