...
fn unique_id_generator_matches_matrixraft_packing() {
    let generator = RustRaftUniqueIdGenerator::new(0x12, 0x3456);
    let id = generator.next();
    assert_eq!(id, 0x12000000345601);

    for offset in 1..1000 {
        assert_eq!(generator.next(), id + offset);
    }

    let decoded = RustRaftUniqueIdGenerator::decode(id);
...
fn unique_id_counter_rolls_into_timestamp_bits() {
    ...
    assert_eq!(RustRaftUniqueIdGenerator::decode(first).timestamp_millis, 8);
...
fn unique_id_generator_is_atomic_for_concurrent_request_ids() {
    let generator = Arc::new(RustRaftUniqueIdGenerator::new(3, 4));
    let mut handles = Vec::new();
    for _ in 0..8 {
        let generator = Arc::clone(&generator);
        handles.push(thread::spawn(move || {
            (0..128).map(|_| generator.next()).collect::<Vec<_>>()
        }));
    }
