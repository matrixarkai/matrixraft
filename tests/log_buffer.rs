...
use matrixraft::{LogBuffer, LogEntry, LogId};
...
fn range(start: u64, end: u64, term: u64) -> Vec<LogEntry> {
    (start..end).map(|index| entry(index, term)).collect()
}

#[test]
fn log_buffer_tracks_range_terms_and_bounded_load() {
    let mut buffer = LogBuffer::new(1000, LogId { term: 0, index: 0 });
    ...
    assert_eq!(buffer.term_at(0), Some(0));
    assert_eq!(buffer.term_at(1), Some(1));
    assert_eq!(buffer.term_at(9), Some(1));
    assert_eq!(buffer.term_at(10), Some(2));
    assert_eq!(buffer.term_at(19), Some(2));

    let loaded = buffer.read_entries(3, 8, 0).expect("load range");
    assert_eq!(
        loaded
            .iter()
    ...
    let bounded = buffer
        .read_entries(1, 19, entry(1, 1).payload.len())
...
fn log_buffer_reset_truncate_and_release_memory() {
    let mut buffer = LogBuffer::new(128, LogId { term: 0, index: 0 });
    buffer
