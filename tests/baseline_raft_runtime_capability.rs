...
#[test]
fn baseline_raft_runtime_capability_report_names_process_path_missing_fields() {
    let mut input = ready_input();
    input
        .data_node_rollout
        .as_mut()
...
#[test]
fn baseline_raft_runtime_capability_report_requires_read_safety_on_both_planes() {
    let mut input = ready_input();
    input
        .metaserver_rollout
        .as_mut()
...
#[test]
fn baseline_raft_runtime_capability_prometheus_exports_generic_metrics() {
    let mut input = ready_input();
    ...
    assert!(metrics.text.contains(
        "rustraft_baseline_raft_capability_ready{plane=\"data_node\",cluster=\"a\\\"b\\\\c\\n\",capability=\"wal_segment_lifecycle\""
    ));
    assert!(metrics.text.contains(
        "rustraft_baseline_raft_capability_field_present{plane=\"data_node\",cluster=\"a\\\"b\\\\c\\n\",capability=\"wal_segment_lifecycle\",field=\"wal.compaction_observed\"} 0"
    ));
    assert!(metrics.text.contains(
        "rustraft_baseline_raft_blocker_present{plane=\"data_node\",cluster=\"a\\\"b\\\\c\\n\",blocker=\"wal_segment_lifecycle:missing:wal.compaction_observed\"} 1"
    ));
    assert!(metrics
        .text
        .contains("rustraft_baseline_raft_missing_capability_count"));
}
