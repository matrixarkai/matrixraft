...
use matrixraft::{
    GenericApplyRequest, GenericApplyResponse, GenericLogEntry, GenericSnapshot,
    GenericSnapshotChunk, GroupId, LogEntry, LogId, LogIndex, NodeId, Payload, SnapshotMetadata,
    SnapshotPayload, Term,
};
...
fn default_log_and_snapshot_models_use_byte_payloads() {
    ...
    let snapshot_payload: SnapshotPayload = vec![4, 5, 6];
...
fn generic_models_accept_domain_payload_and_group_ids() {
    ...
    let entry = GenericLogEntry {
        log_id: LogId { term: 8, index: 13 },
        payload: payload.clone(),
        is_command: true,
    };
    let apply_request = GenericApplyRequest {
        group_id: group_id.clone(),
        log_id: entry.log_id.clone(),
        payload: payload.clone(),
    };
    let apply_response = GenericApplyResponse {
        applied_index: 13,
...
type GenericSnapshotChunkAlias = GenericSnapshotChunk<SnapshotPayload>;
