...
usage() {
  cat <<'USAGE'
Usage: baseline_raft_native_kvbench_adapter.sh --workload ID --node-count 3 --iterations N --batch-size N [--payload-size-bytes N] [--wal-dir PATH] [--snapshot-dir PATH] [--baseline_raft-root PATH]

Runs BaselineRaft's native example/kv kvserver+kvbench path for the workloads that
...
BaselineRaft-vs-RustRaft report remains fail-closed until a complete
baseline_raft_parity_benchmark harness exists.
USAGE
}
...
iterations=128
batch_size=16
payload_size_bytes="${BASELINE_RAFT_NATIVE_RECORD_LENGTH:-128}"
wal_dir=""
snapshot_dir=""
...
blockers = [
    line.strip()
    for line in os.environ.get("BASELINE_RAFT_NATIVE_PREFLIGHT_BLOCKERS", "").splitlines()
    if line.strip()
]
specific = sys.argv[1]
if specific:
...
unsupported_workload_sample() {
  local reason="$1"
  local blocker="benchmark:baseline_raft_native_kvbench_unsupported:$workload:$reason"
  echo "$blocker" >&2
...
case "$workload" in
  ...
  wal_fsync|snapshot_install_catchup|snapshot_streaming|leader_transfer_under_load)
    unsupported_workload_sample "requires full BaselineRaft parity harness"
    exit 0
    ;;
  *)
...
if [[ "$node_count" != "3" ]]; then
  blocker="benchmark:baseline_raft_native_kvbench_requires_three_nodes:$node_count"
  echo "$blocker" >&2
  emit_sample false "$operation_count" 1000000000 1000000000 1.0 "" "" "native-kvbench-partial" "$blocker"
  exit 0
fi

if [[ -z "$baseline_raft_root" || ! -d "$baseline_raft_root" ]]; then
  blocker="benchmark:real_baseline_raft_missing:${baseline_raft_root:-unset}"
  echo "$blocker" >&2
  emit_sample false "$operation_count" 1000000000 1000000000 1.0 "" "" "native-kvbench-partial" "$blocker"
  exit 0
fi

kvserver="${BASELINE_RAFT_KVSERVER_BIN:-$baseline_raft_root/build/example/kv/kvserver}"
kvbench="${BASELINE_RAFT_KVBENCH_BIN:-$baseline_raft_root/build/example/kv/kvbench}"

if [[ ! -x "$kvserver" && ! -f "$kvserver" ]]; then
  blocker="benchmark:baseline_raft_kvserver_binary_missing:$kvserver"
  echo "$blocker" >&2
  emit_sample false "$operation_count" 1000000000 1000000000 1.0 "" "" "native-kvbench-partial" "$blocker"
  exit 0
fi
...
if [[ ! -x "$kvbench" && ! -f "$kvbench" ]]; then
  blocker="benchmark:baseline_raft_kvbench_binary_missing:$kvbench"
  echo "$blocker" >&2
  emit_sample false "$operation_count" 1000000000 1000000000 1.0 "" "" "native-kvbench-partial" "$blocker"
  exit 0
fi

revision="$(git -C "$baseline_raft_root" rev-parse HEAD 2>/dev/null || true)"
build_profile="${BASELINE_RAFT_BUILD_PROFILE:-native-kvbench}"
if [[ -n "$wal_dir" ]]; then
  work_dir="$wal_dir"
elif [[ -n "${BASELINE_RAFT_NATIVE_WORK_DIR:-}" ]]; then
  work_dir="$BASELINE_RAFT_NATIVE_WORK_DIR"
else
  work_dir="$(mktemp -d "${TMPDIR:-/tmp}/rustraft-baseline_raft-kvbench.XXXXXX")"
fi
mkdir -p "$work_dir"
...
cleanup() {
  ...
  if [[ -z "${BASELINE_RAFT_NATIVE_KEEP_WORK_DIR:-}" ]]; then
    rm -rf "$work_dir"
  fi
}
trap cleanup EXIT
...
sleep "${BASELINE_RAFT_NATIVE_STARTUP_SECONDS:-5}"

if ! kill -0 $server_pids >/dev/null 2>&1; then
  blocker="benchmark:baseline_raft_native_cluster_start_failed:$work_dir"
  echo "$blocker" >&2
...
if not matches:
    blocker = f"benchmark:baseline_raft_native_kvbench_parse_failed:{log_path}"
    ...
    sample = {
        ...
        "blockers": [] if int(count) > 0 else ["benchmark:baseline_raft_native_kvbench_zero_operations"],
    }
