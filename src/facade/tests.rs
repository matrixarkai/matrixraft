...
mod tests {
    ...
    fn ready_production_input() -> RustRaftProductionReadinessInput {
        RustRaftProductionReadinessInput {
            ...
            reference_raft_benchmark: Some(RustRaftReferenceRaftBenchmarkEvidence {
                ...
                workloads: crate::benchmark::rustraft_reference_raft_benchmark_workloads()
                    .into_iter()
    ...
    fn ready_benchmark_sample(
        ...
        crate::benchmark::RustRaftBenchmarkSample {
            ...
            implementation: match engine {
                crate::benchmark::RustRaftBenchmarkEngine::ReferenceRaft => {
                    crate::benchmark::RustRaftBenchmarkImplementation::ReferenceRaftCpp
                }
            ...
            harness_kind: match engine {
                crate::benchmark::RustRaftBenchmarkEngine::ReferenceRaft => {
                    crate::benchmark::RustRaftBenchmarkHarnessKind::FullReferenceRaftHarness
                }
                crate::benchmark::RustRaftBenchmarkEngine::RustRaft => {
    ...
    fn ready_benchmark_report() -> crate::benchmark::RustRaftBenchmarkReport {
        let comparisons = crate::benchmark::rustraft_reference_raft_benchmark_workloads()
            .into_iter()
            .map(|workload| {
                let reference_raft = ready_benchmark_sample(
                    workload,
                    crate::benchmark::RustRaftBenchmarkEngine::ReferenceRaft,
                    crate::benchmark::RustRaftBenchmarkEngineSource::RealReferenceRaft,
                    100,
                    200,
                    1_000.0,
                );
        ...
        crate::benchmark::RustRaftBenchmarkReport {
            ...
            required_workloads: crate::benchmark::rustraft_reference_raft_benchmark_required_workloads(),
            passed: true,
            comparisons,
        }
    }
    ...
    fn production_readiness_gate_accepts_validated_benchmark_artifacts() {
        ...
        input.reference_raft_benchmark = None;
        let benchmark = ready_benchmark_report();
        let summary =
            crate::benchmark::rustraft_reference_raft_benchmark_failure_summary(&benchmark);
        ...
        assert!(report
            .satisfied
            .contains(&"benchmark:real_reference_raft".to_string()));
        assert!(report
            .satisfied
    ...
    fn production_readiness_artifact_gate_does_not_claim_real_evidence_for_empty_workloads() {
        ...
        let report_evidence = crate::benchmark::rustraft_reference_raft_benchmark_evidence(&benchmark);
        assert!(!report_evidence.real_reference_raft);
        assert!(!report_evidence.rustraft_runtime);
        assert!(!report_evidence.reference_raft_cpp_reference);
        ...
        let summary_evidence =
            crate::benchmark::rustraft_reference_raft_benchmark_evidence_from_summary(&summary);
        assert!(!summary_evidence.real_reference_raft);
        assert!(!summary_evidence.rustraft_runtime);
        assert!(!summary_evidence.reference_raft_cpp_reference);
    ...
    fn production_readiness_artifact_gate_rejects_stale_report_schema() {
        ...
        input.reference_raft_benchmark = None;
        let mut benchmark = ready_benchmark_report();
        benchmark.schema = "rustraft.reference_raft_benchmark_report.v0".to_string();
        let summary =
            crate::benchmark::rustraft_reference_raft_benchmark_failure_summary(&benchmark);
        ...
        assert!(report.production_blockers.iter().any(|blocker| {
            blocker
                == "benchmark:report_schema_mismatch:rustraft.reference_raft_benchmark_report.v0:rustraft.reference_raft_benchmark_report.v1"
        }));
    }
    ...
    fn production_readiness_artifact_gate_rejects_sample_shape_mismatch() {
        ...
        input.reference_raft_benchmark = None;
        let mut benchmark = ready_benchmark_report();
        benchmark.comparisons[0].reference_raft.node_count = 1;
        benchmark.comparisons[0].reference_raft.payload_size_bytes = 512;
        benchmark.comparisons[0].reference_raft.implementation =
        ...
        assert!(report.production_blockers.iter().any(|blocker| {
            blocker == "single_key_writes:benchmark:reference_raft_sample_node_count_mismatch:1:5"
        }));
        assert!(report.production_blockers.iter().any(|blocker| {
            blocker == "single_key_writes:benchmark:reference_raft_sample_payload_size_mismatch:512:4096"
        }));
        assert!(report.production_blockers.iter().any(|blocker| {
            blocker == "single_key_writes:benchmark:reference_raft_implementation_mismatch:model:reference_raft_cpp"
        }));
        assert!(report.production_blockers.iter().any(|blocker| {
    ...
    fn production_readiness_artifact_gate_rejects_invalid_latency_order() {
        ...
        input.reference_raft_benchmark = None;
        let mut benchmark = ready_benchmark_report();
        benchmark.comparisons[0].reference_raft.p50_latency_micros = 300;
        benchmark.comparisons[0].reference_raft.p99_latency_micros = 200;
        let summary =
            crate::benchmark::rustraft_reference_raft_benchmark_failure_summary(&benchmark);
        ...
        assert!(report.production_blockers.iter().any(|blocker| {
            blocker
                == "single_key_writes:benchmark:reference_raft_sample_latency_order_invalid:300:200"
        }));
    }
    ...
    fn production_readiness_gate_rejects_stale_summary_schema() {
        ...
        let mut summary =
            crate::benchmark::rustraft_reference_raft_benchmark_failure_summary(&benchmark);
        summary.schema = "rustraft.reference_raft_benchmark_summary.v0".to_string();
        let input = rustraft_production_readiness_input_with_benchmark_summary(input, &summary);
        ...
        assert!(report.production_blockers.iter().any(|blocker| {
            blocker
                == "benchmark:summary_schema_mismatch:rustraft.reference_raft_benchmark_summary.v0:rustraft.reference_raft_benchmark_summary.v1"
        }));
    }
    ...
    #[test]
    fn production_readiness_gate_rejects_summary_without_full_reference_raft_harness() {
        let mut input = ready_production_input();
        input.reference_raft_benchmark = None;
        let benchmark = ready_benchmark_report();
        let mut summary =
            crate::benchmark::rustraft_reference_raft_benchmark_failure_summary(&benchmark);
        summary.workloads[0].reference_raft_harness_kind =
        ...
        assert!(report.production_blockers.iter().any(|blocker| {
            blocker
                == "benchmark:summary_reference_raft_full_harness_missing:single_key_writes:native_kvbench_partial"
        }));
    }
    ...
    fn production_readiness_gate_requires_every_benchmark_workload() {
        ...
        assert!(report
            ...
            .iter()
            .any(|action| action.contains("every required ReferenceRaft-vs-RustRaft")));
    }
    ...
    fn production_readiness_gate_rejects_generic_benchmark_blockers() {
        ...
        assert!(report
            .recommended_next_actions
            .iter()
            .any(|action| action.contains("clear all ReferenceRaft-vs-RustRaft")));
    }
    ...
    fn production_readiness_gate_fails_closed_without_runtime_evidence() {
        ...
        assert!(report
            .production_blockers
            .contains(&"benchmark:real_reference_raft_missing".to_string()));
        assert!(report
            .production_blockers
    ...
    fn production_readiness_gate_rejects_model_only_benchmark_evidence() {
        let mut input = ready_production_input();
        input.reference_raft_benchmark = Some(RustRaftReferenceRaftBenchmarkEvidence {
            real_reference_raft: false,
            rustraft_runtime: false,
            reference_raft_cpp_reference: false,
            rustraft_rust_candidate: false,
            correctness_passed: true,
            performance_within_threshold: true,
            workloads: vec!["single_key_writes".to_string()],
            blockers: vec![
                "benchmark:model_reference_raft:single_key_writes".to_string(),
        ...
        assert!(report
            .production_blockers
            .contains(&"benchmark:reference_raft_cpp_reference".to_string()));
        assert!(report
            .production_blockers
            .contains(&"benchmark:rustraft_rust_candidate".to_string()));
        assert!(report
            .production_blockers
            .contains(&"benchmark:model_reference_raft:single_key_writes".to_string()));
    }
    ...
    fn production_readiness_gate_reports_classified_benchmark_blockers() {
        let mut input = ready_production_input();
        let missing_binary =
            "single_key_writes:benchmark:reference_raft_kvserver_binary_missing:/tmp/reference_raft/kvserver"
                .to_string();
        let unsupported_workload =
            "lease_reads:benchmark:reference_raft_native_kvbench_unsupported:lease_reads"
                .to_string();
        let correctness_blocker =
            "read_index_reads:benchmark:reference_raft_native_kvbench_zero_operations".to_string();
        ...
        input.reference_raft_benchmark = Some(RustRaftReferenceRaftBenchmarkEvidence {
            ...
            missing_reference_raft_binaries: vec![missing_binary.clone()],
        ...
        assert!(!report.ready);
        for blocker in [
            "benchmark:reference_raft_binaries_missing",
        ...
        for action in [
            "build or configure the real ReferenceRaft benchmark binaries before claiming parity",
            "implement full ReferenceRaft harness coverage for unsupported workloads",
            "fix ReferenceRaft/RustRaft benchmark correctness failures before performance gating",
            "fix RustRaft benchmark p50/p99/throughput regressions against ReferenceRaft",
        ] {
            assert!(
                report
                    .recommended_next_actions
