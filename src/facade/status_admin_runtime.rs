...
pub fn rustraft_production_readiness_report(
    ...
    require_reference_raft_benchmark(
        input.reference_raft_benchmark.as_ref(),
        &mut satisfied,
        &mut missing,
        &mut production_blockers,
        &mut recommended_next_actions,
...
fn require_reference_raft_benchmark(
    benchmark: Option<&RustRaftReferenceRaftBenchmarkEvidence>,
    ...
    require_option(
        ...
        blockers,
        actions,
        "attach real ReferenceRaft-vs-RustRaft benchmark evidence",
    );
    ...
    for (present, id, action) in [
        (
            benchmark.real_reference_raft,
            "benchmark:real_reference_raft",
            "run the benchmark against a real ReferenceRaft binary",
        ),
        (
        ...
        (
            benchmark.reference_raft_cpp_reference,
            "benchmark:reference_raft_cpp_reference",
            "run the benchmark against the real C++ MatrixRaft reference implementation",
        ),
        (
        ...
        (
            ...
            "bring RustRaft p50/p99 latency and throughput within the configured ReferenceRaft threshold",
        ),
    ] {
        require_bool(present, id, satisfied, missing, blockers, actions, action);
    }
    for workload in benchmark::rustraft_reference_raft_benchmark_workloads() {
        ...
        require_bool(
            ...
            blockers,
            actions,
            "run every required ReferenceRaft-vs-RustRaft parity workload",
        );
    }
    let required_workloads = benchmark::rustraft_reference_raft_benchmark_workloads();
    ...
    require_bool(
        ...
        blockers,
        actions,
        "provide exactly the canonical ReferenceRaft-vs-RustRaft workload set",
    );
    require_benchmark_blocker_category(
        ...
        blockers,
        actions,
        "clear all ReferenceRaft-vs-RustRaft benchmark blockers before claiming production parity",
    );
    require_benchmark_blocker_category(
        "benchmark:reference_raft_binaries_missing",
        &benchmark.missing_reference_raft_binaries,
        satisfied,
        missing,
        blockers,
        actions,
    ...
    for blocker in &benchmark.blockers {
        ...
        require_bool(
            ...
            blockers,
            actions,
            "clear uncategorized ReferenceRaft benchmark blocker",
        );
    }
}
...
fn require_fault_harness(
    ...
    require_option(
        ...
        blockers,
        actions,
        "attach ReferenceRaft-style fault harness evidence from real RustRaft process runs",
    );
    ...
    require_bool(
        ...
        blockers,
        actions,
        "run all required ReferenceRaft fault scenarios against RustRaft",
    );
    ...
    for result in &fault_harness.results {
        ...
        require_bool(
            ...
            blockers,
            actions,
            "clear ReferenceRaft fault scenario blocker",
        );
    }
    ...
    for blocker in &fault_harness.missing {
        ...
        require_bool(
            ...
            blockers,
            actions,
            "clear ReferenceRaft fault harness blocker",
        );
    }
}
...
pub fn rustraft_reference_raft_runtime_capability_report(
    input: &RustRaftProductionReadinessInput,
) -> RustRaftReferenceRaftRuntimeCapabilityReport {
...
pub fn rustraft_reference_raft_runtime_capability_prometheus(
    report: &RustRaftReferenceRaftRuntimeCapabilityReport,
    labels: &[(&str, &str)],
) -> RustRaftPrometheusMetricSet {
    let mut out = String::new();
    let mut metric_count = 0;
    out.push_str("# HELP rustraft_reference_raft_ready Whether ReferenceRaft-derived runtime capability evidence is complete.\n");
    out.push_str("# TYPE rustraft_reference_raft_ready gauge\n");
    push_prometheus_metric(
        &mut out,
        "rustraft_reference_raft_ready",
        labels,
        u64::from(report.ready),
    );
    metric_count += 1;

    out.push_str("# HELP rustraft_reference_raft_capability_ready ReferenceRaft-derived runtime capability readiness by family.\n");
    out.push_str("# TYPE rustraft_reference_raft_capability_ready gauge\n");
    for capability in &report.capability_evidence {
        ...
        push_prometheus_metric(
            &mut out,
            "rustraft_reference_raft_capability_ready",
            &metric_labels,
        ...
        for evidence in &capability.evidence {
            ...
            push_prometheus_metric(
                &mut out,
                "rustraft_reference_raft_capability_field_present",
                &evidence_labels,
    ...
    out.push_str(
        "# HELP rustraft_reference_raft_blocker_present ReferenceRaft-derived runtime capability blockers.\n",
    );
    out.push_str("# TYPE rustraft_reference_raft_blocker_present gauge\n");
    for blocker in &report.blockers {
        let mut blocker_labels = labels.to_vec();
        blocker_labels.push(("blocker", blocker.as_str()));
        push_prometheus_metric(
            &mut out,
            "rustraft_reference_raft_blocker_present",
            &blocker_labels,
            1,
        );
        metric_count += 1;
    ...
    push_prometheus_metric(
        &mut out,
        "rustraft_reference_raft_satisfied_capability_count",
        labels,
    ...
    push_prometheus_metric(
        &mut out,
        "rustraft_reference_raft_missing_capability_count",
        labels,
    ...
    push_prometheus_metric(
        &mut out,
        "rustraft_reference_raft_blocker_count",
        labels,
