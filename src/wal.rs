...
pub struct WalLifecycleStatus {
    ...
    #[serde(default)]
    pub slow_fsync_segment_count: u64,
    #[serde(default)]
    pub compacted_slow_fsync_segment_count: u64,
}
...
pub struct WalLifecycleEvidenceValidationReport {
    ...
    pub compaction_after_slow_fsync_observed: bool,
    #[serde(default)]
    pub released_segment_count_match: bool,
    #[serde(default)]
    pub compacted_after_slow_fsync_count_match: bool,
    #[serde(default)]
...
pub fn matrixraft_wal_lifecycle_evidence(status: &WalLifecycleStatus) -> WalLifecycleEvidence {
    WalLifecycleEvidence {
        ...
        compaction_after_slow_fsync_observed: status.compacted_after_slow_fsync_count > 0
            && status.compacted_slow_fsync_segment_count > 0
            && status.slow_fsync_segment_count >= status.compacted_slow_fsync_segment_count
            && status.released_segment_count >= status.compacted_after_slow_fsync_count
            && status.slow_fsync_count > 0,
        released_segment_count: status.released_segment_count,
        compacted_after_slow_fsync_count: status.compacted_after_slow_fsync_count,
        slow_fsync_segment_count: status.slow_fsync_segment_count,
        compacted_slow_fsync_segment_count: status.compacted_slow_fsync_segment_count,
    }
}
...
pub fn matrixraft_validate_wal_lifecycle_evidence_artifact(
    ...
    let released_segment_count_match =
        recomputed.released_segment_count == artifact.evidence.released_segment_count;
    let compacted_after_slow_fsync_count_match = recomputed.compacted_after_slow_fsync_count
        == artifact.evidence.compacted_after_slow_fsync_count;
    ...
    for (present, requirement) in [
        ...
        (released_segment_count_match, "released_segment_count_match"),
        (
